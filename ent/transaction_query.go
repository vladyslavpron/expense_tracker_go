...
func (tq *TransactionQuery) QueryBalance() *BalanceQuery {
	query := (&BalanceClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) QueryCategory() *CategoryQuery {
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withBalance:  tq.withBalance.Clone(),
...
func (tq *TransactionQuery) WithBalance(opts ...func(*BalanceQuery)) *TransactionQuery {
	query := (&BalanceClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withBalance = query
	return tq
}

// WithCategory tells the query-builder to eager-load the nodes that are connected to
// the "category" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithCategory(opts ...func(*CategoryQuery)) *TransactionQuery {
	query := (&CategoryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := tq.withBalance; query != nil {
		if err := tq.loadBalance(ctx, query, nodes, nil,
			func(n *Transaction, e *Balance) { n.Edges.Balance = e }); err != nil {
			return nil, err
		}
	}
...
func (tq *TransactionQuery) loadBalance(ctx context.Context, query *BalanceQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Balance)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		fk := nodes[i].BalanceID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "balance_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransactionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withBalance != nil {
			_spec.Node.AddColumnOnce(transaction.FieldBalanceID)
		}
